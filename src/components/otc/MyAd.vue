    ...
    <div class="nav-right">
        <div class="col-xs-12 col-md-10 padding-right-clear">
            <div style="text-align:left;margin:10px 0;">
                <Button v-if="certifiedBusinessStatus==2" icon="plus-round" type="primary" @click="publish">{{$t('otc.myad.post')}}</Button>
            </div>
...
export default {
  ...
  methods: {
    ...
    changePage(pageNo) {
      ...
      this.$http.post(this.host + "/otc/advertise/all",params).then(response => {
        ...
        if (resp.code == 0) {
          resp.data.content
            ? (this.tableAdv = resp.data.content.filter(item => {
                return item.status != 2;
              }))
            : (this.tableAdv = resp.data.filter(item => {
                return item.status != 2;
              }));
          this.loading = false;
  ...
  computed: {
    ...
    // certifiedBusinessStatus: function() {
    //   return this.$store.getters.certifiedBusinessStatus;
    // }
  },
  watch: {
    lang: function() {
  ...
  created() {
    this.getAd();
    this.$http.post(this.host + "/uc/approve/certified/business/status").then(response => {
        var resp = response.body;
        if (resp.code == 0) {
          this.certifiedBusinessStatus=resp.data.certifiedBusinessStatus
        } else {
        }
      });
  }
