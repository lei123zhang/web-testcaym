            ...
            <div class="login_right" style="padding:20px 30px;">
                <Form v-if="allowRegister" ref="formInline" :model="formInline" :rules="ruleInline" inline>
                    ...
                    <FormItem prop="country">
                        ...
                        <div style="font-size:14px;padding-left:10px;">{{$t('uc.regist.countryTip')}}</div>
                    </FormItem>
                    ...
                    <FormItem prop="repassword">
                        <Input type="password" v-model="formInline.repassword" :placeholder="$t('uc.regist.againPwd')">
                    ...
                    <FormItem>
                        ...
                        <div v-show="!showCode" style="font-size:14px;padding-left:10px;">{{$t('uc.regist.emailTip')}}</div>
                    </FormItem>
                    
                    <div style="display:flex;justify-content:center;align-items:center;" >
                        <span style="font-size:14px;line-height:32px;padding-right:6px;">{{$t('uc.regist.hasaccount')}}</span>
                        <router-link to="login" style="font-size:14px;line-height:32px;">
                            <span style="font-size:14px;line-height:32px;">{{$t('uc.regist.login')}}</span>
                        </router-link>
                    </div>
                </Form>
  ...
  data() {
    const validateUser = (rule, value, callback) => {
      if (this.changeActive == 0) {
        ...
        if (value == "" && this.formInline.country === "中国") {
          callback(new Error(this.$t("uc.regist.teltip")));
        } else if (
          !reg.test(this.formInline.user) &&
    ...
    const validatePassword = (rule, value, callback) => {
      ...
      if (value === "") {
        callback(new Error(this.$t("uc.regist.pwdtip")));
      } else if (!reg.test(this.formInline.password)) {
        callback(new Error(this.$t("uc.regist.hybrid")));
      } else {
        callback();
      }
    };
