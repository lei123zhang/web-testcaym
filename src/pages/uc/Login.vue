            ...
            <div class="login_right" style="padding:80px 30px;">
                <Form ref="formInline" :model="formInline" :rules="ruleInline" inline>
                    ...
                    <div style="display:flex;justify-content:center;align-items:center;" >
                        <span style="font-size:14px;line-height:32px;padding-right:6px;">{{$t('uc.login.noaccount')}}</span>
                        <router-link to="register" style="font-size:14px;line-height:32px;">
                            <span style="font-size:14px;line-height:32px;">{{$t('uc.login.register')}}</span>
                        </router-link>
                    </div>
                </Form>
            </div>
        ...
        window.initGeetest(
          ...
          function(captchaObj) {
            captchaObj.onSuccess(function() {
              ...
              var result = that._captchaResult;
              if (!result) {
                $("#notice").show();
                setTimeout(function() {
                  $("#notice").hide();
                ...
                that.$refs.formInline.validate(valid => {
                  if (valid) {
                    var params = {};
                    params["username"] = that.formInline.user;
                    params["password"] = that.formInline.password;
                    that.$http
                      .post(that.host + that.api.uc.login, params)
                      .then(response => {
                        var resp = response.body;
                        if (resp.code == 0) {
                          that.$Message.success(that.$t("uc.login.success"));
                          that.$store.commit("setMember", response.body.data);
                          if (
                            that.$route.query.key != null &&
