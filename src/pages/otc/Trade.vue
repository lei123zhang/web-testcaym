<template>
    <div class="nav-right tradeCenter">
        <section class="trade-group merchant-top">
            ...
            <a v-show="certifiedBusinessStatus!=2" href="/#/identbusiness">{{$t("otc.applymerchant")}}&gt;&gt;</a>
        </section>
    ...
    return {
      ...
      advertiment: {
        ...
        columns: [
          ...
          {
            title:self.$t('otc.certification'),
            key:'level',
            align:'center',
            render:function(h,params){
              return params.row.level===1?self.$t('otc.realCerti'):self.$t('otc.merchCerti')
            }
          },
          {
          ...
          },
          {
            ...
            align:'center',
            render: function(h, params) {
             let arr =  params.row.payMode.split(',')
             return arr.map((item)=>{
                if(item==='支付宝'){
                  return  self.$t('otc.publishad.zfb') 
                }else if(item==='银联'){
                  return self.$t('otc.publishad.unionpay')
                }
                else if(item==='微信'){
                  return self.$t('otc.publishad.wx')
                }
              })
            }
          },
  ...
  methods: {
    ...
    getMerchants() {
      if(this.isLogin){
        ...
        .then(res => {
          that.$store.commit("certiStatus", res.body.data.certifiedBusinessStatus);
